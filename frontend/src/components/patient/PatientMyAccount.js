...
function PatientMyAccount() {
  ...
  const handleLogout = async () => {
    setLoggingOut(true);
    try {
      await fetch('/qualified-doctors/patient/logout', { method: 'POST' });
    } catch {
      // proceed with client-side logout even if request fails
    } finally {
      sessionStorage.removeItem('currentUser');
      navigate('/qualified-doctors/patient/register');
    }
  };
...
export default PatientMyAccount;
